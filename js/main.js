...
function shipClick(evt) {
  ...
  var clickedSpaceId =
    clickedSpace.id.charAt(5) +
    clickedSpace.id.charAt(6) +
    clickedSpace.id.charAt(7);
  var clickedSpaceIdIndex = board1.indexOf(clickedSpaceId);
  console.log("clickedSpaceId ", clickedSpace);
  console.log("clickedSpaceIdIndex ", clickedSpaceIdIndex);
  ...
  if (clickedSpace.style.backgroundColor !== "black") {
    ...
    if (shipArray[0] !== null && shipArray[1] === null) {
      if (
        (parseInt(shipArray[0].charAt(1) + shipArray[0].charAt(2)) ===
          parseInt(rowNum) + 1 &&
...
document
  .getElementById("compBoard")
  .addEventListener("click", toggleToCompBoard);
  ...
  for (compShipName in compShips) {
    ...
    if (vertOrHor === 1) {
      function vertical() {
        ...
        vertRandRow = Math.floor(
          Math.random() * (11 - compShips[compShipName].length) + 1
        );
        /*iterate through the rand row to the length of the ship */
        for (
          i = vertRandRow;
          i < vertRandRow + compShips[compShipName].length;
          i++
        ) {
          ...
          if (compShipSpaces.includes(`${vertRandCol}` + i)) {
            ...
            compShips[compShipName].splice(
              compShips[compShipName].indexOf(null),
              1,
              `${vertRandCol}` + i
            );
      ...
      function horizontal() {
        ...
        horColIndex = Math.floor(
          Math.random() * (11 - compShips[compShipName].length)
        );
        horRandCol = letters[horColIndex];
        for (
          var i = horColIndex;
          i < horColIndex + compShips[compShipName].length;
          i++
        ) {
          /*if the compShipSpaces array already contains the random generated
...
function compBoardClick(evt) {
  ...
  for (compShipName in compShips) {
    ...
    if (
      compShips[compShipName].every(x => {
        return x === "hit";
      })
    ) {
    ...
    if (
      checkWinnerArray.includes("randCarrier") &&
...
function compTurn() {
  ...
  let hit = previousGuesses[previousHits.lastIndexOf("hit")];
  let hitIndex = board1.indexOf(hit);
  let up = document.getElementById("user-" + board1[hitIndex - 10]);
  let down = document.getElementById("user-" + board1[hitIndex + 10]);
  let right = document.getElementById("user-" + board1[hitIndex + 1]);
  let left = document.getElementById("user-" + board1[hitIndex - 1]);
  let hitSpot = document.getElementById("user-" + hit);

  //initial guess
  if (previousGuesses.length === 0) {
    guess = randGuess;
  } else if (
    (previousHits[previousHits.length - 1] === "hit" ||
      (previousHits[previousHits.length - 2] === "hit" &&
        previousHits[previousHits.length - 1] === "miss") ||
      (previousHits[previousHits.length - 3] === "hit" &&
        previousHits[previousHits.length - 2] === "miss" &&
        previousHits[previousHits.length - 1] === "miss") ||
      (previousHits[previousHits.length - 4] === "hit" &&
        previousHits[previousHits.length - 3] === "miss" &&
        previousHits[previousHits.length - 2] === "miss" &&
        previousHits[previousHits.length - 1] === "miss")) &&
    (previousGuesses[previousHits.lastIndexOf("hit")].charAt(0) !== "A" ||
      previousGuesses[previousHits.lastIndexOf("hit")].charAt(0) !== "J") &&
    previousGuesses[previousHits.lastIndexOf("hit")].charAt(1) !== "1"
  ) {
    if (
      ...
      guess = board1[hitIndex - 10];
    } else if (
      right.style.backgroundColor !== "red" &&
      right.style.backgroundColor !== "white"
    ) {
      guess = board1[hitIndex + 1];
    } else if (
  ...
  for (ShipName in userShips) {
    ...
    if (
      compCheckWinnerArray.includes("carrier") &&
...
function reset() {
  ...
  for (i = 0; i < 100; i++) {
    if (
      document.getElementById(`user-${board1[i]}`).style.backgroundColor !==
      "black"
    ) {
      document.getElementById(`user-${board1[i]}`).style.backgroundColor =
        "transparent";
